...
public class WatchList {
	...
	private static Accessor<RegistryNamespaced, ObjectIntIdentityMap> intRegistryAccess = new Accessor<RegistryNamespaced, ObjectIntIdentityMap>("field_148759_a");
	...
	public static class WatchableBlock implements Watchable {
		...
		public WatchableBlock(Block toWatch) {
			...
			blockName = Block.blockRegistry.getNameForObject(toWatch).toString();
			if (blockName == null)
				throw new RuntimeException(toWatch.toString() + " currently unregistered");
		}
		...
		public ProblemReport problemReport() {
			...
			if (newID != blockID) {
				problem = true;
				if (newID == -1)
					result += block.toString() + " lacks ID";
				if (newID != -1)
					result += block.toString() + " moved from " + blockID + " to " + newID;
			}
	...
	public static class WatchableItem implements Watchable {
		...
		public WatchableItem(Item paired) {
			...
			if (itemID == -1)
				throw new RuntimeException(item.toString() + " currently unregistered");
			itemName = Item.itemRegistry.getNameForObject(item).toString();
			if (itemName == null)
				throw new RuntimeException(item.toString() + " currently unregistered");
		}
		...
		public ProblemReport problemReport() {
			...
			if (newID != itemID) {
				problem = true;
				if (newID == -1)
					result += item.toString() + " lacks ID";
				if (newID != -1)
					result += item.toString() + " moved from " + itemID + " to " + newID;
			}
